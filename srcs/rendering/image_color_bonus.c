...
/*   Updated: 2021/03/24 17:28:34 by lvirgini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_image	*get_image_black(t_app *app, char *pixels, int nb_pixel)
{
	...
	while (x + 4 <= nb_pixel)
	{
		...
		res->pixels[x + RED_PIXEL] = (unsigned char)c.r;
		res->pixels[x + GREEN_PIXEL] = (unsigned char)c.g;
		res->pixels[x + BLUE_PIXEL] = (unsigned char)c.b;
