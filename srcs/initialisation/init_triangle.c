...
/*   Updated: 2021/01/25 16:42:34 by lvirgini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_triangle	*malloc_triangle(t_vec3 pos[3], t_color color, int texture)
{
	...
	triangle->determinant = (triangle->norme2_ab * triangle->norme2_ac) -
							(triangle->dot_ab_ac * triangle->dot_ab_ac);
	if (!(triangle->plane = malloc_plane(triangle->a, triangle->ab,
							triangle->color, 0)))
		return (NULL);
	return (triangle);
}
